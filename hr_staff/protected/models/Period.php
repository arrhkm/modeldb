...
class Period extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id', 'required'),
			array('period_name', 'ECompositeUniqueValidator'),
			array('id', 'numerical', 'integerOnly'=>true),
			array('period_name', 'length', 'max'=>15),
			array('date_start, date_end', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, date_start, date_end, period_name', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'dayoffs' => array(self::HAS_MANY, 'Dayoff', 'period_id'),
			'payrolls' => array(self::HAS_MANY, 'Payroll', 'period_id'),
		);
	}
